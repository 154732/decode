...
public class QRScanController {
    ...
    public String login(String uuid, Model model, HttpSession session){
        model.addAttribute("uuid",uuid);
        return "ok";
    }
    ...
    @RequestMapping("/pool")
    @ResponseBody
    public JSONObject pool(String uuid,HttpSession session){
        ...
        Object PoolCache = session.getAttribute("login");
        try {
            if (pool == null) {
                // 扫码超时,进线程休眠
